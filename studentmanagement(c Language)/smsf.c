...
void add(){
	...
	for(i=0;i<count;i++){
	    ...
	    fprintf(fp,"\n %d    %d   \t%s            \t%.2f          \t%c",st+1,s[st].roll,s[st].name,s[st].percentage,s[st].grade);
...
void update(){
	...
	switch(choice){
		case 1:
			...
			scanf("%s",&nnm);
			for(i=0;i<st;i++){
				if(id==fscanf(fu,"%d",&s[i].roll)){
					strcpy(s[i].name,nnm);
					fprintf(fu,"%s",s[i].name);
				}
			}
			break;
		case 2:
